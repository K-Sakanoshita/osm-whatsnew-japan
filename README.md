...
- 必要なときにメイン地図と切り替えて使える都道府県選択マップと、代表タグの種別選択による絞り込み
- 全国および都道府県別の件数、代表タグ、マッパー、日別更新数、変更セットの集計
- 代表タグごとのマッパー内訳と、新規・更新件数の表示
- 選択期間に更新された対象地物を約1kmメッシュ単位で集計する更新密度マップ（GPSによる現在地移動に対応）
...
「地域選択」を押すとメイン地図が都道府県選択マップに切り替わり、都道府県の選択後は対象地域へ移動した通常地図に戻ります。
...
- 描画対象は取得データ全体から時系列順に均等抽出し、モバイル最大2,000件、デスクトップ最大4,000件
- 画面上の約8px単位で近接する地物を一つの光へまとめ、件数が多い光ほど少し大きく明るく表示
...
- デモの描画上限をモバイル2,000件、デスクトップ4,000件とし、画面上で近接する地物を一つの光へ集約しました。サンプリング結果のキャッシュとCanvas描画により負荷を抑えています。
