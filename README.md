...
プロフィール画面では、同系列の獲得済みバッジを「Ⅰ・Ⅱ・Ⅲ」のように1枚へまとめて表示します。バッジ条件も同じ系列単位にまとめ、例えば「千葉県で10件、50件、250件マッピング」と表示します。最近の活動はデスクトップで18件を3列表示し、モバイルでは6件を初期表示して`details`／`summary`から残りを展開します。
