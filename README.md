...
- 全国および都道府県別の件数、代表タグ、マッパー、日別更新数、日別マッパー人数、変更セットの集計と、グラフ単位の新規／更新フィルター
- 分析画面の変更セット一覧・グラフから、対象の変更セットをEasy Changeset Viewerで直接表示
- 代表タグごとのマッパー内訳と、新規・更新件数の表示
- 選択期間に更新された対象地物を約1kmメッシュ単位で集計する更新密度（初期値は直近2週間、GPSによる現在地移動に対応）
- 期間、都道府県、マッパー、代表タグ、新規・更新種別によるAPI検索
- 地図データのGeoJSON・CSV出力、分析画面の各リストの個別CSV出力、表示条件を復元できる地図URLの共有
- 地図・分析・密度の各画面間でメインマップの中心位置とズームを共有
- マッパー別の今月・累積レベル、統合バッジ、活動地域、最近の活動、関連マッパーを表示するプロフィール
- バッジの獲得条件・獲得者数、レベルアップ条件をプロフィール内の共通モーダルで表示
- 全国または地図で選択した都道府県について、最終活動日時が新しいマッパーを一覧表示

Webアプリケーションは `www/whatsnew/` にあります。

公開画面の`index.html`、`details.html`、`coverage.html`、`profile.html`では、Google Analytics 4（測定ID：`G-V4357J57C7`）を使用してアクセス状況を計測します。Analytics Cookieは現在のホストだけで利用し、認証付きの管理画面`admin.php`は計測対象外です。
...
| `modal.js` / `modal.css` | 各画面で使用するモーダルの共通生成API・開閉・デザイン |
| `api.php` | JSON APIの統合エンドポイント |
| `profile-sync.php` | マッパープロフィールのCLI集計バッチ |
| `profile.html` / `profile.js` / `profile-lib.php` | マッパー別の直近1年間プロフィール、レベル・バッジ定義 |
...
地図の凡例は、低ズームのヒートマップ／クラスタ表示では「新規・更新件数」、高ズームのアイコン表示では新規・更新マーカーへ自動的に切り替わります。選択期間が0件でも操作パネルを開くボタンを残し、期間を変更できるようにしています。
...
タイムラインの「対象」では「新規・更新」「新規のみ」「更新のみ」を切り替えられ、変更時はタイムスケールを先頭へ戻します。共有ボタンは地図概要の「デモ」ボタン右側にあり、コピーしたURLを開くと期間、地域、地図位置、再生間隔を復元します。データの読み込み後はタイムスケールの先頭で停止し、強調表示された再生ボタンを押すと再生を開始します。
...
外部DBへ接続できない開発PCでは、ローカル完結のDockerテスト環境を使用できます。MariaDBとphpMyAdminはDockerで起動し、WebはホストのPHP内蔵サーバーから作業ツリーの`www/whatsnew/`を直接参照します。

起動にはDocker Engine、Docker Composeプラグイン、`php`、`curl`、`start-stop-daemon`と、PHP拡張`pdo_mysql`、`mbstring`、`SimpleXML`が必要です。Ubuntu／DebianでPHP 8.5を使う場合は、少なくとも次のPHPパッケージを用意します。

```shell
sudo apt install php8.5-cli php8.5-mysql php8.5-mbstring php8.5-xml
docker compose version
```

リポジトリのルートで次のコマンドを実行すると、MariaDB、phpMyAdmin、PHP開発Webサーバーを起動し、スキーマ、テストデータ、プロフィール集計を準備します。

```shell
./scripts/test-env-docker.sh start
./scripts/test-env-docker.sh restart
./scripts/test-env-docker.sh status
./scripts/test-env-docker.sh logs
./scripts/test-env-docker.sh stop
```

通常は最初に`start`を実行します。設定や起動状態をまとめて作り直す場合は`restart`、動作確認は`status`、ログ確認は`logs`、終了は`stop`を使用します。

起動時に最新の`schema.sql`を再適用し、未投入の場合だけプロフィール・地図確認用の架空データを自動投入します。既存データがあるローカルDBでも、衝突しないテスト専用IDで共存します。公開環境用の`private-osm-config.php`は使用・変更しません。
...
起動メッセージ後にWebへ接続できない場合は、PID、待受ポート、Webログ、APIを順に確認します。

```shell
./scripts/test-env-docker.sh status
./scripts/test-env-docker.sh logs
ss -ltnp | grep ':8000'
curl -fsS 'http://127.0.0.1:8000/api.php?mode=japan&days=1'
```

管理された実行環境などでバックグラウンドのホストPHPが終了する場合は、通常の端末から次のコマンドを実行してWebサーバーを維持できます。`/absolute/path/to/OSMWhatNewJapan`は実際のリポジトリの絶対パスへ置き換えてください。MariaDBコンテナは先に起動しておきます。

```shell
OSM_APP_CONFIG=/absolute/path/to/OSMWhatNewJapan/docker/test-host/private-osm-test-config.php \
OSM_TEST_DB_PORT=3307 \
php -S 0.0.0.0:8000 -t /absolute/path/to/OSMWhatNewJapan/www/whatsnew
```

自動投入される`docker/test-db/profile-test-data.sql`は、実行日を基準にした直近52週の継続活動、断続活動、初心者、JSTの週境界を再現します。直近2日には707件を用意し、通常の地図・集計画面でも十分な件数を確認できます。さらに、今月の活動が1・10・50・100・250・500件の専用マッパーを用意し、月間レベルを確認できます。月間レベルは1,000件のLv.7「さいこうマッパー」まで定義されています。座標は北海道・東京都・神奈川県・愛知県・大阪府・広島県・福岡県の陸上へ分散します。投入済みかどうかは`osm_sync_state`の`profile_test_data_version`で判定し、通常の再起動では重複投入しません。
...
プロフィール画面は集計済みテーブルだけを参照するため、画面アクセスごとの全件集計は行いません。OSMの公開プロフィール画像URLも集計時に取得し、30日間キャッシュします。プロフィール上部と関連マッパー一覧はこの保存済みURLを使用し、画像がない場合や取得に失敗した場合は表示名の頭文字へフォールバックします。画像確認は未取得または確認日時が古いユーザーから1回最大200人（100人単位、最大2リクエスト）に制限し、複数回の同期へ分散します。人数は非公開設定の`profile_avatar_refresh_limit`で0～1000人の範囲に変更できます。ローカルDocker環境では`test-env-docker.sh start`がテストデータ投入後にプロフィール集計も自動実行します。本番環境ではOSM同期処理の後に`profile-sync.php`をcron等から実行してください。
...
| `prefecture_code` | 都道府県コード `01`～`47` の2桁（例：大阪府は `27`）。内部で都道府県名に変換。`prefecture` と併記する場合は同じ都道府県に限る |
...
`prefecture` と `prefecture_code` の両方を省略すると全国です。コード指定時もDBとレスポンスの都道府県名は従来どおりで、`meta.filters` には指定コードと解決した都道府県名を返します。不正なコードや名前との不一致はHTTP 400です。各モードでの都道府県フィルターの適用範囲は従来どおりです。

`from` と `to` は日本時間の暦日として受け付け、検索時にUTCへ変換します。レスポンス内の日時とDB内の日時はUTCを基準に扱い、ブラウザー画面ではJSTへ変換して表示します。
...
| `tag_key` | 保存済みnodeの全タグから、指定キーの存在を検索。空でないUTF-8文字列、最大255文字 |
| `tag_value` | `tag_key` の値の完全一致。UTF-8文字列、最大255文字。空文字も検索可能。単独指定はHTTP 400 |

直近30日で大阪府で新規作成・更新された `playground=*` のnodeを取得する例：

```text
api.php?mode=pois&days=30&prefecture_code=27&tag_key=playground
api.php?mode=pois&days=30&prefecture_code=27&tag_key=playground&tag_value=swing
```

タグ検索は `mode=pois` のみ対応し、他モードでの指定はHTTP 400です。キー・値は大文字小文字や前後の空白を区別し、ワイルドカードは展開しません。存在検索には `tag_key=playground` を指定し、`tag_value=*` は指定しません。`category` / `category_value` は従来の代表カテゴリ検索で、タグ条件および他の条件とはANDで組み合わせます。タグ未指定時は従来どおりway・relationも取得対象です。

`action` 省略時は新規・更新の両方が対象です。結果は収集済みnodeの最新状態で、更新履歴や公園との包含関係を返すものではありません。続きの取得では同じ検索条件を維持して `cursor` に `meta.nextCursor` を指定します。タグ検索は期間・都道府県で候補を絞ってもJSON評価が必要なため、全国・長期間での高頻度取得には注意してください。
...
- 獲得済みバッジ、次のバッジ候補、バッジ条件、条件ごとの獲得者数
- 今月の編集件数・活動日数、月間Lv.1〜7、累積Lv.1〜18とレベルアップ条件
- 上位カテゴリ、上位都道府県、最近の対象地物18件
- 同じ地域で活動する日替わりの関連マッパー9人（保存済みプロフィール画像URLを含む）

集計値はこのアプリケーションが現在保持する収集対象地物に限られ、OSMでの全編集活動や生涯実績を表すものではありません。初回集計時に条件を満たしていたバッジは、正確な過去の取得日時を復元せず`backfill`（確認）として返します。

`badgeDefinitions`の各要素には、表示順に使う`badgeLevel`と、失効していない獲得者数`awardCount`を含みます。

プロフィール画面では、同系列の獲得済みバッジを「Ⅰ・Ⅱ・Ⅲ」のように1枚へまとめて表示します。獲得バッジは内部レベルの高い順（Ⅴ、Ⅳ、Ⅲ、Ⅱ、Ⅰ）、同じレベルでは条件件数の多い順に表示します。「発見の巨匠」「情報整備の巨匠」「活動300日達成」「トレジャーハンター」はⅣ、「伝説の発見者」「伝説のメンテナー」「活動365日達成」はⅤとして扱います。

獲得バッジをクリックすると、そのカードが代表する最高段階のバッジについて、失効していない所持者数とマッパー一覧をモーダル表示します。一覧は必要なときだけ次のAPIから取得します。

```text
api.php?mode=badge_mappers&badge_key=tag_shopping_2
```

バッジ条件も同じ系列単位にまとめ、例えば「千葉県で10件、50件、250件マッピング」と表示します。「次のバッジ」横の`❗`からは、現在の全バッジ条件と獲得者数を確認できます。レベル条件、獲得バッジ条件、サイトガイドは共通モーダルの外枠・閉じるボタン・モバイル表示を使用します。最近の活動はデスクトップで18件を3列表示し、モバイルでは6件を初期表示して`details`／`summary`から残りを展開します。

プロフィールは地図画面の地物一覧・詳細ポップアップ、分析画面のマッパー名、またはマッパー画面の検索・最近更新したマッパー一覧から開きます。マッパー画面の地図では分析画面と同じ都道府県境界を使用し、全国または選択した都道府県で活動したマッパーを最終活動日時の新しい順に表示します。

### 最近更新したマッパー：`mode=profile_region_mappers`

```text
api.php?mode=profile_region_mappers
api.php?mode=profile_region_mappers&prefecture=千葉県
```

直近1年間の集計対象から、全国または指定した都道府県で活動したマッパーを最終活動日時の新しい順で最大100件返します。レスポンスにはUID、表示名、新規・更新・合計件数、最終活動日時を含みます。
...
- プロフィールの獲得バッジをクリックすると、同じバッジの所持者数と縦スクロール可能なマッパー一覧をモーダル表示するようにしました。

### 2026-09-05

- 地図データの読み込み後と共有URLの復元後はタイムラインを自動再生せず、先頭で停止して再生ボタンを強調表示するようにしました。
- さくらの共有ドメインでAnalytics Cookieが不正なドメインとして拒否されないよう、公開4画面のGA4 Cookieを現在のホスト専用にしました。
- 地図とリストのリサイズ操作をウィンドウ全体で追跡し、Firefoxなどで終了時に無効なpointer IDの例外が発生しないようにしました。

### 2026-08-18

- 分析画面の「変更セット別（降順）」で、詳細一覧の変更セット番号とグラフの項目名からEasy Changeset Viewerを直接開けるようにしました。

### 2026-08-15

- 分析画面に、選択期間の日ごとのユニークなマッパー人数を示すグラフを追加しました。全国集計APIの日別データには後方互換の`mapperCount`を追加しています。
- 公開4画面へGoogle Analytics 4（GA4）を設定しました。認証付き管理画面は計測対象外です。

### 2026-08-14

- 獲得バッジを内部レベルの高い順、同レベルでは条件件数の多い順に表示するよう変更し、巨匠・伝説・活動300／365日・トレジャーハンターへⅣ／Ⅴ相当の内部レベルを設定しました。
- プロフィールAPIのバッジ定義へ条件ごとの獲得者数を追加し、「次のバッジ」横の`❗`から現在のバッジ条件一覧を確認できるようにしました。
- サイトガイド、レベル条件、バッジ条件のモーダル外枠・開閉・デザイン・閉じるボタンを`modal.js`／`modal.css`の共通基盤へ統一しました。モバイルでは上下左右8pxの余白を残して最大表示します。
- プロフィール上部のOSMプロフィールリンクをユーザー名の右側へ移動し、表示を`↗`に簡略化しました。「次のバッジ」見出しと`❗`のサイズ・縦位置も調整しました。
- 地図表示モードに応じて凡例をヒートマップ／クラスタの「新規・更新件数」と新規・更新アイコンへ切り替えるようにしました。期間内のPOIが0件でも操作パネルを開けるようにしました。
- Docker開発環境の必要コマンドとPHP拡張、起動・確認・停止方法、ホストPHPが終了する場合の手動起動方法を追記しました。

### 2026-08-13

- マッパープロフィールをレスポンシブに再構成し、活動サマリーを縦画面では2列、横画面では4列で表示するコンパクトなレイアウトへ変更しました。
- 今月・累積レベルの条件を共通デザインのモーダルへ移し、月間Lv.7「さいこうマッパー」（1,000件）を追加しました。
- 同系列の獲得バッジと条件を「Ⅰ・Ⅱ・Ⅲ」形式で統合し、カードから確認日・取得日・更新日を非表示にしました。
- 最近の活動を18件・デスクトップ3列とし、モバイルでは最初の6件を表示して既存の`details`／`summary`から展開できるようにしました。日時表示は年月までに簡略化しました。
- 関連マッパーを最大9人表示し、保存済みプロフィール画像の遅延読み込みと頭文字フォールバックに対応しました。
- マッパー画面の全国・都道府県一覧を「最近更新したマッパー」に統一し、最終活動日時の新しい順へ変更しました。都道府県境界と選択線も分析画面と同じ色・太さへ統一しました。
